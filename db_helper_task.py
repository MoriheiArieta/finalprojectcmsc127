...
def userLogin():
    ...
    if(userResult is None):
        ...
        try:
            ...
            cur.execute("SELECT user_id, username FROM user WHERE email = ? AND password = ?", (email, password))
            result = cur.fetchone()
...
def addCategory(userId):
    ...
    try: 
        cur.execute("SELECT category_id FROM category WHERE category_name = ?", (categoryName,))
        categoryResult = cur.fetchone()
        if(categoryResult is not None):
            print("Category already exists! Please enter another category name")
        else:
            cur.execute("INSERT INTO category (category_name, creation_date, user_id) VALUES (?, ?, ?)", (categoryName,now, userId))
            conn.commit()
    except mariadb.Error as e: 
        print(f"Error: {e}")
...
def addTask(userId):
    ...
    try:
        ...
        if(taskResult is not None):
            ...
            cur.execute("INSERT INTO task (task_name, task_details, task_date, task_completed, user_id) VALUES (?, ?, ?, ?, ?)", (taskName, taskDetails, now, "No", userId))
            conn.commit()
    ...
    if(taskResult is None):
        ...
        try: 
            ...
            if(result is not None):
                ...
                cur.execute("UPDATE task SET task_name = ?, task_details = ? WHERE task_id = ?", (newTaskName, newTaskDetails, foundTaskId))   
                conn.commit()
...
def markTaskDone(userId):
    ...
    if(taskResult is None):
        ...
        try: 
            cur.execute("SELECT task_id FROM task WHERE task_name = ? AND task_completed = 'No'", (findTask,))
            result = cur.fetchone()
            if(result is not None):
                foundTaskId = result[0]

                cur.execute("UPDATE task SET task_completed = 'Yes' WHERE task_id = ?", (foundTaskId,))   
                conn.commit()
        ...
        try: 
            ...
            if(allTasks is not None):
                for index, task in enumerate(allTasks, start=1):
                    ...
                    cur.execute("SELECT category_name FROM category WHERE category_id = ?", (task[6],))
